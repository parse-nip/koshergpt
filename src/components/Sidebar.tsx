...
import type { Conversation, StudyMode } from '../types/chat';
...
function modeBadgeLabel(mode: StudyMode): string {
  return mode === 'chavrusa' ? 'Chavrusa' : 'Research';
}

export function Sidebar({ conversations, activeId, mode, onSelect, onNew }: SidebarProps) {
    ...
    <aside className="flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden border-r border-parchment-dark bg-parchment/60">
      ...
      <div className="p-3">
        ...
        >
          ...
          {meta.newSessionLabel}
        </Button>
      </div>

      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overflow-x-hidden px-3 pb-3">
        <p className="px-2 py-2 font-sketch text-base text-ink/35">Recent sessions</p>
            ...
            conversations.map((conv) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1 text-left">
                  <span className="mb-0.5 block font-body text-[10px] uppercase tracking-wide text-ink/35">
                    {modeBadgeLabel(conv.mode ?? 'research')}
                  </span>
                  <span className="block font-body text-[13px] leading-snug line-clamp-3 text-left">
                    {conv.title}
                  </span>
                </span>
              </Button>
      ...
      <div className="border-t border-parchment-dark px-4 py-3">
        <p className="text-center font-sketch text-base text-ink/30">{meta.sidebarTagline}</p>
      </div>
    </aside>
  );
}
